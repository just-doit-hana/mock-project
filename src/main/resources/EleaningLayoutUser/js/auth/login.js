function register() {
    ...
    axios({
        url: 'http://localhost:8082/api/user/auth/register',
        method: 'POST',
        data: userLogin
    })
        .then(function(response){
            swal("Good job!", "Register SuccessFull", "success").then(function(resp){
...
function login() {
    ...
    axios({
        url: 'http://localhost:8082/api/user/auth',
        method: 'POST',
        data: userLogin
    })
        .then(function(response){
...
function getInfo(){
    ...
    axios({
        ...
        `;
            document.getElementById('abc123').innerHTML = `
             <input type="hidden" id="useID123" value="${resp.data.user_id}">
            `;
